...
def test_showtime_movie_partial_serializer(showtimes_list):
    ...
    assert "id" in data[0]
    assert data[0]["movie"][0]["genre"]["name"] == "Action"
...
def test_showtime_retrieve_serializer(showtime_f1_london):
    ...
    assert data["theater_name"] == "Room 1"
...
@pytest.mark.django_db
def test_showtime_seat_status_serializer(showtime_f1_london):
    serializer = ShowtimeSeatStatusSerializer(showtime_f1_london)
    data = serializer.data
...
@pytest.mark.django_db
def test_showtime_report_serializer(showtime_f1_london):
    serializer = ShowtimeReportSerializer(showtime_f1_london)
    data = serializer.data
    ...
    assert "id" in data
    assert "movie" not in data
    assert "movie_title" not in data
    assert "city" not in data
