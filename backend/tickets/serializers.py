...
class BookingPartialSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Booking
        fields = ["id", "showtime", "seat", "status", "booked_at", "expires_at"]
...
class BookingCreateSerializer(serializers.ModelSerializer):
    ...
    seat_ids = serializers.ListField(
        child=serializers.IntegerField(write_only=True, min_value=0), min_length=1
    )
    ...
    def validate(self, data):
        ...
        if status not in allowed_statuses:
            raise serializers.ValidationError(
                {"status": "Only reserved or pending_payment allowed."}
            )
        ...
        conflicting = set(
            Booking.objects.filter(showtime=showtime, seat_id__in=seat_ids)
            .exclude(
                status__in=[
...
class BookingListPaymentSerializer(serializers.ModelSerializer):
    ...
    showtime_price = serializers.DecimalField(
        source="showtime.price", max_digits=8, decimal_places=2
    )
